...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        previous_state_levels = self.s_levels[level]
        self.a_levels.append(set())

        for action in self.all_actions:
            action_node = PgNode_a(action)
            if action_node.prenodes.issubset(previous_state_levels):
                for state_node in previous_state_levels:
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        effect_1 = set(node_a1.action.effect_add)
        effect_2 = set(node_a2.action.effect_rem)

        if len(effect_1.intersection(effect_2)) > 0:
            return True
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        effect_1_add = set(node_a1.action.effect_add)
        effect_1_rem = set(node_a1.action.effect_rem)

        precond_1_pos = set(node_a1.action.precond_pos)
        precond_1_neg = set(node_a1.action.precond_neg)

        effect_2_add = set(node_a2.action.effect_add)
        effect_2_rem = set(node_a2.action.effect_rem)

        precond_2_pos = set(node_a2.action.precond_pos)
        precond_2_neg = set(node_a2.action.precond_neg)

        return effect_1_add & precond_2_neg | effect_1_rem & precond_2_pos | effect_2_add & precond_1_neg | effect_2_rem & precond_1_pos
    ...
    def h_levelsum(self) -> int:
        ...
        for goal in goals:
            ...
            s_level_list = enumerate(self.s_levels)
            for level, s_nodes in s_level_list:
                if node in s_nodes:
